...
def find_urls(
    ...
    # 1. finding all by selecting from tag <a up to href=
    # 2. create group that does not contain " or # before reaching a new " or #
    reg = re.compile(r"<a.*href=[\"]([^\"#]+)[\"|#]", flags=re.IGNORECASE)
    urls = set()

    # 1. find all the anchor tags, then
    # 2. find the urls href attributes

    for i in reg.findall(html): 
        if i[1] == '/':
            i = "https:" + i
        elif i[0] == '/' and base_url != None:
            i = base_url + i
